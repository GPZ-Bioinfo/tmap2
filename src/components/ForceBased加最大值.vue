...
export default {
  ...
  created() {
    ...
    apolloClient
      ...
      .then((response) => {
        ...
        let sizeValueMax = (parseFloat(_this.sizeValueMax) / 5).toFixed(1)
        this.updateChart()
  ...
  methods: {
              ...
              itemStyle: {
                normal: {
                  color: function (params) {
                    _this.BarColorList = _this.colorList1
          ...
          series: [
            {
              ...
              itemStyle: {
                normal: {
                  color: function (params) {
                    _this.BarColorList = _this.colorList2
                    const colorList = _this.BarColorList
                    if (params.dataIndex >= colorList.length) {
                      params.dataIndex = params.dataIndex - colorList.length
                    }
          ...
          series: [
            {
              ...
              itemStyle: {
                normal: {
                  color: function (params) {
                    _this.BarColorList = _this.colorList3
                    const colorList = _this.BarColorList
                    if (params.dataIndex >= colorList.length) {
                      params.dataIndex = params.dataIndex - colorList.length
                    }
    ...
    histogramChange(param) {
      ...
      //           normal: {
      //             color: function (params) {
      //               _this.BarColorList = _this.colorList0
      //               const colorList = _this.BarColorList
      //               if (params.dataIndex >= colorList.length) {
      //                 params.dataIndex = params.dataIndex - colorList.length
      //               }
    ...
    updateChart() {
      ...
      const option = {
        ...
        xAxis: {
          ...
          data: this.xData,
          axisLabel: {
            interval: Math.ceil(this.xData.length / this.intervalCount)
          }
        },
        yAxis: {
          name: 'nodes number',
    ...
    generateXData() {
      ...
      this.intervalCount = Math.ceil(max / intervalValue)
      return Array.from({ length: _this.intervalCount }, (_, index) => (index + 0.5) * intervalValue)
    },
    generateYData() {
      // 生成随机的纵坐标数据，确保与横坐标等分数量相匹配
      return Array.from({ length: this.intervalData.length }, () => Math.random() * 100)
    },
    ...
    updateNodeColor() {
      ...
      this.node.attr('fill', function (d, i) {
        const index = _this.xData.indexOf(Math.round(d.id / 50))
